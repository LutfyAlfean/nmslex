...
import { Globe, Server, Wifi, Shield, AlertTriangle, CheckCircle, XCircle, ZoomIn, ZoomOut, RotateCcw } from "lucide-react";
...
export default function Topology() {
  const [nodes, setNodes] = useState<TopoNode[]>(defaultNodes);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    setNodes(prev => prev.map(n =>
      n.id === dragging
        ? { ...n, x: Math.max(30, Math.min(svgW - 30, svgP.x)), y: Math.max(30, Math.min(svgH - 30, svgP.y)) }
        : n
    ));
  }, [dragging]);
  ...
  const resetPositions = () => setNodes(defaultNodes);
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Visualisasi koneksi antar agent dan server NMSLEX — drag node untuk mengatur posisi</p>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={resetPositions} className="p-1.5 rounded-lg bg-secondary/50 hover:bg-secondary text-foreground transition-colors" title="Reset positions">
        ...
        <div className="flex gap-5">
              ...
              >
                <defs>
                  ...
                  <style>{`
                    ...
                    .link-active { animation: dash-flow 1s linear infinite; }
                  `}</style>
                </defs>
                ...
                {links.map((link, i) => {
                  ...
                  return (
                    <g key={i}>
                      <line x1={from.x} y1={from.y} x2={to.x} y2={to.y} stroke={color} strokeWidth={link.status === "active" ? 1.5 : 1} opacity={opacity} strokeDasharray={link.status === "down" ? "4 4" : link.status === "degraded" ? "6 3" : "5 5"} className={link.status === "active" ? "link-active" : ""} />
                      {link.label && (
                        <text x={(from.x + to.x) / 2} y={(from.y + to.y) / 2 - 6} textAnchor="middle" fill="hsl(220, 10%, 50%)" fontSize="8" fontFamily="monospace">{link.label}</text>
                      )}
                    </g>
                  );
                })}
                ...
                {nodes.map((node) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <circle cx={node.x} cy={node.y} r={size} fill={sc.bg} stroke={sc.stroke} strokeWidth={isSelected ? 2.5 : 1} opacity={node.status === "disconnected" ? 0.5 : 1} />
                      <text x={node.x} y={node.y + 1} textAnchor="middle" dominantBaseline="central" fontSize={node.type === "master" ? 18 : 14}>{typeIcons[node.type]}</text>
                      <text x={node.x} y={node.y + size + 14} textAnchor="middle" fill="hsl(180, 10%, 85%)" fontSize="10" fontWeight="500">{node.label}</text>
                      <text x={node.x} y={node.y + size + 25} textAnchor="middle" fill="hsl(220, 10%, 50%)" fontSize="8" fontFamily="monospace">{node.ip}</text>
          ...
          <div className="w-72 flex-shrink-0 space-y-4">
            <div className="glass rounded-xl p-4">
              <h3 className="text-sm font-semibold text-foreground mb-3">{selected ? "Node Detail" : "Select a node"}</h3>
              {selected ? (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    {selected.status === "active" ? <CheckCircle className="w-4 h-4 text-success" /> : selected.status === "disconnected" ? <XCircle className="w-4 h-4 text-destructive" /> : <AlertTriangle className="w-4 h-4 text-warning" />}
                    <span className={`text-xs capitalize font-medium ${selected.status === "active" ? "text-success" : selected.status === "disconnected" ? "text-destructive" : "text-warning"}`}>{selected.status}</span>
                  </div>
                ...
                <p className="text-xs text-muted-foreground">Klik atau drag node di peta untuk melihat detail koneksi.</p>
              )}
            </div>
